...
#+DATE: [2021-01-01 Fri]
#+OPTIONS: author:t creator:t email:t toc:t num:nil
...
*** Annotated 2D datasets

We use a simple tool =gif2h5= to create a dataset representation conforming to the
[[https://portal.hdfgroup.org/display/HDF5/HDF5+Image+and+Palette+Specification%2C+Version+1.2][HDF5 image specification]]. As a sample image, we use the sample trajectory from
section [[sec:viz][Visualization]]. Unfortunately, the simple =gif2h5= tool accepts only GIF
images, and we need to first convert the PNG file =timeseries_vis.png=.
...
=timeseries_vis.h5= contains a 2D dataset of pixels called =Image0= and a 2D palette
called =global=.
...
/t_x                     Dataset {100/100, 2/2}
    ...
    Storage:   1600 logical bytes, 1600 allocated bytes, 100.00% utilization
    Type:      native double
/~obj_pointed_by_347952  Dataset {128/128, 3/3}
    Attribute: CLASS scalar
        Type:      8-byte null-terminated ASCII string
    Attribute: PAL_VERSION scalar
        Type:      4-byte null-terminated ASCII string
