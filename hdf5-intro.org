...
- The community provides excellent, in some cases several, bindings for Python,
  R, C++, Julia, .NET, Java...
...
int main(int argc, char** argv)
{
  ...
  file = H5Fcreate("single-proc.h5", H5F_ACC_TRUNC, H5P_DEFAULT, fapl);
  H5Pclose(fapl);
...
int main(int argc, char** argv)
{
...

  {
    ...
    file = H5Fcreate("multi-proc.h5", H5F_ACC_TRUNC, H5P_DEFAULT, fapl);
    H5Pclose(fapl);
...
lambda(hid_t, (hid_t file, const char* name, hsize_t elt_count),
       {
         hid_t result;
         hid_t fspace = H5Screate_simple(1, (hsize_t[]) { elt_count }, NULL);
